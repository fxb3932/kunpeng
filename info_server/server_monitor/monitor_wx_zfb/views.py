...
def testyh(request):
    ...
    test = 'aaa'
    print(test)
    return render(request, 'monitor11/testyh.html', req)
...
(select sum(trans_count_all)  as now from monitor_wx_zfb_trans_flow where settle_date=date_format(now(),'%Y%m%d')
and txn_time < now()) a,
(
select sum(trans_count_all) as ago from monitor_wx_zfb_trans_flow where settle_date=date_format(now(),'%Y%m%d')
and txn_time < date_sub(now(),interval 4 minute))  b;      
            '''
            cursor.execute(sql)
...
where settle_date >  date_sub(curdate(),interval 7 day)
...
def get_R1_TransPlatMin(request):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            #         else '兴业网联' END 'hz_tran_plat'
            #         from monitor_wx_zfb_trans_flow
            #         where settle_date='20191111'
            #         and txn_time >= '2019-11-11 00:00:00'
            #         and txn_time <= '2019-11-11 02:00:00'
            #
            #     ) m
            #     group by date_format(txn_time,'%H:%i'),hz_tran_plat order by 1;
            # '''
...
(select sum(trans_amt_all)/1  as amt_now from monitor_wx_zfb_trans_flow where settle_date=date_format(now(),'%Y%m%d')
and txn_time < now()) a,
(
select sum(trans_amt_all)/1 as amt_ago from monitor_wx_zfb_trans_flow where settle_date=date_format(now(),'%Y%m%d')
and txn_time < date_sub(now(),interval 5 minute))  b;          
            '''
            cursor.execute(sql);
            result = cursor.fetchall();
            print(result)
...
(
    ...
    from monitor_wx_zfb_trans_flow
    where settle_date=date_format(now(),'%Y%m%d')) a
group by a.tran;      
            '''
            cursor.execute(sql);
            result = cursor.fetchall();
